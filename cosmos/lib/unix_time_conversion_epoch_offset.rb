...
module OpenC3
  ...
  class UnixTimeConversionEpochOffset < Conversion
    ...
    def call(value, packet, buffer)
      if @microseconds_item_name
        return (Time.at(packet.read(@seconds_item_name, @seconds_type, buffer), packet.read(@microseconds_item_name, @microseconds_type, buffer)).sys + @epoch_offset_seconds)
      else
        return (Time.at(packet.read(@seconds_item_name, @seconds_type, buffer), 0).sys + @epoch_offset_seconds)
      end
    end
